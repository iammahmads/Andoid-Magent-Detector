...
class Scanner : ComponentActivity(), SensorEventListener {
    ...
    private fun toggleScanning() {
        if (isScanning) {
            ...
            btnToggleSensor.backgroundTintList = ColorStateList.valueOf(colorNeonGreen)
            ...
            btnToggleSensor.backgroundTintList = ColorStateList.valueOf(colorCyan) // Changed tint to Cyan for active state
        }
    ...
    private fun showThresholdInfoDialog() {
        // 1. Create the builder with your custom style
        val builder = AlertDialog.Builder(this, R.style.AlertDialogCustom)

        builder.setTitle("What is the Threshold?")
            .setMessage(
                "The Threshold is the minimum magnetic field magnitude that must be exceeded to trigger a 'Metal Detected!' warning.\n\n" +
                        "1. Tap CALIBRATE in an empty area.\n" +
                        "2. Drag the slider to set sensitivity.\n" +
                        "3. The red line shows the current threshold."
            )
        ...
        // 2. Optional: Force the width to be 90% of the screen so it looks like a proper "box"
    ...
    private fun doVibrate() {
        // 1. Use the class-level 'vibrator' variable you already initialized in onCreate
        val v = vibrator

        if (v == null || !v.hasVibrator()) {
            showToast("ERROR: Device hardware has no vibrator!")
            return
        }

        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                // 2. INCREASE INTENSITY: Use 255 instead of DEFAULT_AMPLITUDE
                // Many phones have a very weak 'Default'
                val effect = VibrationEffect.createOneShot(300, 255)

                // 3. CHANGE USAGE: Use USAGE_ALARM
                // This is the highest priority and often bypasses "Silent" or "Do Not Disturb"
            ...
            showToast("Vibrate failed: ${e.message}")
        }
    }

    private fun showToast(message: String) {
        Toast.makeText(this, "[SYSTEM]: $message", Toast.LENGTH_SHORT).show()
    }
}